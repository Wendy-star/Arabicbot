...
const connect = async () => {
   ...
   global.client = Socket({
      ...
      browser: ['ItsukaBotz', 'safari', '1.0.0'],
      auth: state,
      ...
      if (connection === 'connecting') {
       spinnies.add('start', {
         ...
         spinnies.succeed('start', {
            text: `Connected, you login as ${client.user.name || client.user.verifiedName} #Hero.js`
         })
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      let text_welcome = `مرحبا بك يا +tag نورتنا في +grup group.`
      let text_left = `+tag تطلع يجي غيرك وفرت علينا اوكسجين.`
