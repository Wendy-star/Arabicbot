...
module.exports = async (client, m, plugins, store) => {
   try {
      ...
      if (m.isGroup && !groupSet.stay && (new Date * 1) >= groupSet.expired && groupSet.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 البوت خلاص انحلب و يغادر المجموعة.', null, {
      ...
      if (users && (new Date * 1) >= users.expired && users.expired != 0) {
         return client.reply(m.chat, Func.texted('italic', '🚩 خدمة النخبة انتهت المرجوا الكلام مع المطور.')).then(async () => {
            users.premium = false
            users.expired = 0
      ...
      if (m.isGroup && !m.isBot && users && users.afk > -1) {
         client.reply(m.chat, `مرحبا بك لقد استيقظت من وضع النوم : ${Func.texted('bold', Func.toTime(new Date - users.afk))}\n\n• ${Func.texted('bold', 'سبب النوم')}: ${users.afkReason ? users.afkReason : '-'}`, m)
         users.afk = -1
         users.afkReason = ''
      }
      cron.schedule('00 00 * * *', () => {
      ...
      if (((m.isGroup && !groupSet.mute) || !m.isGroup) && !users.banned) {
         if (body && body == isPrefix) {
            ...
            if (!banchat) {
               ...
               await client.reply(m.chat, Func.texted('bold', `جاري . . .`), m)
               return client.reply(m.chat, Func.texted('bold', `سرعة الرد : ${((new Date - old) * 1)}ms (nonaktif)`), m)
            }
         }
      }
      ...
      if (body && isPrefix && commands.includes(command) || body && !isPrefix && commands.includes(command) && setting.noprefix || body && !isPrefix && commands.includes(command) && global.evaluate_chars.includes(command)) {
         ...
         if (!commands.includes(command) && matcher.length > 0 && !setting.self) {
            if (!m.isGroup || (m.isGroup && !groupSet.mute)) return client.reply(m.chat, `🚩 الأمر غلط :\n\n${matcher.map(v => '➠ *' + (isPrefix ? isPrefix : '') + v.string + '* (' + v.accuracy + '%)').join('\n')}`, m)
         }
         if (setting.error.includes(command) && !setting.self) return client.reply(m.chat, Func.texted('bold', `🚩 الامر _${(isPrefix ? isPrefix : '') + command}_ محذوف.`), m)
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 سولف مع المطور و اشتري ١٠٠٠ حدود استخدام ضمن باقة النخبك.\n\nاذا تبي تشترك سولف مع المطور*${prefixes[0]}المطور*`, m, {
               largeThumb: true,
            ...
            if (cmd.limit && users.limit > 0) {
               ...
               if (users.limit >= limit) {
                  ...
                  client.reply(m.chat, Func.texted('bold', `🚩 جدود استخدامك للبوت انتهت.`), m)
                  continue
               }
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 اشتري باقة النخبة للمزيد من المعلومات أرسل *#النخبة**`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
